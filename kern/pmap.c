...
page_init(void)
{
	...
	pages[1].pp_link = 0;//page 0 is in use;
	
	struct Page* pg_in_use_start = pa2page(IOPHYSMEM);
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (!(*page_dir_entry & PTE_P)) {
		...
		page->pp_ref++;
		*page_dir_entry = page2pa(page) | PTE_P | PTE_U | PTE_W;
		result = page2kva(page);
	}
	else {
		result = KADDR(PTE_ADDR(*page_dir_entry));
	}
	return result + PTX(va);
	//we must return kernal virtual address
	//but we save physical address in pde and pte.
}
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
	...
	pp->pp_ref++;//make sure pp->pp_ref won't be 0 if pp is re-inserted at the same va.
...
page_remove(pde_t *pgdir, void *va)
{
	...
	if (page==NULL) return;
	if (!(*page_table_entry & PTE_P)) return;
	page_decref(page);
	*page_table_entry = 0;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	// LAB 3: Your code here.
	int va1 = (int)va;
	int up = ROUNDUP((va1+len), PGSIZE);
	
	for (; va1<up; va1 = ROUNDDOWN(va1+PGSIZE, PGSIZE)) {
		
		pte_t * pte = pgdir_walk(env->env_pgdir, (void *)va1, 0);
		if ((!pte)||(~(*pte)&(perm|PTE_P))||va1>=ULIM)
		{
